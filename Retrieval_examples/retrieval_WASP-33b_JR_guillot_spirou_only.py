...
walker_file_out = Path('walker_steps_v07254_wasp33_nights1-2_pc2-4_mask_wings95-90_tr12_HRR_WL_no_disso_short.h5')
...
init_mode = 'continue'  # Options: "burnin", "continue"
...
params_prior = {}
params_prior['abund'] = ['range', -12.0, -0.5]
params_prior['temp'] = ['range', 400, 4500]
# params_prior['cloud'] = ['range', -5.0, 2]
# params_prior['rpl'] = ['range', 1.0, 1.6]
params_prior['kp'] = ['range', 225-30, 225+30]
params_prior['rv'] = ['range', -30, 30]
...
special_treatment = {'kp': ['range', 223, 228],
                     'rv': ['range', -1, 4]}
...
if __name__ == '__main__':
    ...
    with Pool(n_pcu) as pool:
        ...
        sampler.run_mcmc(pos, 10000, progress=False)  # , skip_initial_state_check=True)
